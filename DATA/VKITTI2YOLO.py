...
def convert_annotations(lines):
    ...
    return yolo_annotations, camera_id
...
for scene_folder in os.listdir(VKITTI_LABELS):
    ...
    if os.path.isdir(scene_folder_path) and scene_folder not in excluded:
        ...
        for subfolder in os.listdir(scene_folder_path):
            ...
            # Select subfolders that are not in excluded
            if os.path.isdir(subfolder_path) and subfolder not in excluded:
                ...
                # Check if "bbox.txt" file exists
                    if os.path.isfile(bbox_file_path):
                        ...
                        yolo_annotations, camera_id = convert_annotations(lines)
                        ...
                        for (frame, camera_id), annotations in yolo_annotations.items(): 
                            ...
                            # Manage frame counters for each camera
                            if camera_id not in camera_frame_counters:
                                camera_frame_counters[camera_id] = 0
                            else:
                                camera_frame_counters[camera_id] += 1
                            
                            frame_filename = f"{camera_frame_counters[camera_id]:05d}"  # Use the camera's frame counter
                            
                            # Create a file name based on the format you provided using underscores
                            output_file_name = f"{scene_folder}_{subfolder}_camera{camera_id}_{frame_filename}.txt"
